...
The payoff is measured on the ports, not asserted: a new verb is **four appended declarations inside one block folder** and no production site outside it (TypeScript needs a fifth — a hand-kept type predicate its compiler will not check for you; Kotlin keeps four but authors two of them in the shared core, because it seals a hierarchy within one module, so one folder is two directories); a new state variant is one append plus **three sites the compiler names for you**, all in the same folder. Each port states its own measured counts — for a verb, for an effect kind and for a whole block — beside the command that recounts them, in its own README. A session re-folds from its committed bytes alone, a human override is free (it is the same command with a different actor), and a single bad inference cannot fire an irreversible effect.
