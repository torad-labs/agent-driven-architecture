...
| | |
|---|---|
| **The book** — [`index.html`](index.html) | The complete reference: the inversion, the signed command bus, the stateless-reducer agent, ports and adapters, the provided-but-swappable spine, tiered cognition, concurrency and barge-in, replay and recovery, and the enforced invariants (G1–G14), with a fixed nomenclature. |
| **The worked example** — [`example/`](example/index.html) | One running application — a support-ticket triage console — traced through every seam, Clean-Architecture style. An overview plus seven seam chapters, each running the same eight-slot template and ending in a "what breaks" anti-example. |
...
- **GitHub Pages** (once enabled for this repo): <https://torad-labs.github.io/agent-driven-architecture/> for the book, and `…/example/` for the worked example.
- **Locally**: clone the repo and open `index.html` in any browser. No build step, no dependencies — the only external resources are a web font, a syntax highlighter, and a diagram renderer loaded from a CDN, and each degrades gracefully offline.
...
│   ├── 01-state-and-fold.html
│   ├── 02-the-boundary.html
│   ├── 03-tools-and-context.html
│   ├── 04-projection-and-surface.html
│   ├── 05-ports-and-swap.html
│   ├── 06-blocks-and-root.html
│   ├── 07-replay-and-advanced.html
...
- **The code** — the example's HTML/CSS/JavaScript and the pseudocode snippets — is licensed under the [**MIT License**](LICENSE-CODE). Use it in anything, including commercially; keep the copyright notice.
